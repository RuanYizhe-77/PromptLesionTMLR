...
from monai.data import DataLoader, Dataset, list_data_collate, DistributedSampler, CacheDataset,PersistentDataset
...
def get_loader(args):
    ratios=[0.01, 0.01, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000, 1000000000]
    ...
    if args.phase == 'train':
        ...
        for item in args.dataset_list:
            ...
            if item == 'coronary_data_fold':
                for line in open(os.path.join(args.data_txt_path,  item, 'real_lesion_train_{}.txt'.format(args.fold))):
                    name = line.strip().split('\t')[0]

                    train_img.append(args.data_root_path + name+ '/ct.nii.gz')
                    train_lbl.append(args.data_root_path + name+ '/segmentations/plaque.nii.gz')
                    train_name.append(name)
            else:
                for line in open(os.path.join(args.data_txt_path,  item, 'real_tumor_train_{}.txt'.format(args.fold))):
                    name = line.strip().split()[1].split('.')[0]
                    train_img.append(line.strip().split()[0])
                    train_lbl.append(line.strip().split()[1])
                    train_name.append(name)
                    # import nibabel as nib
                    # label=nib.load(line.strip().split()[1])
