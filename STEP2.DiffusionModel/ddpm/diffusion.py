...
class McmaskEmbedding(nn.Module):
    def __init__(self, in_channels=3, out_channels=12, hidden_dim=64, num_w_layers=3):
        """
...
class PromptGenBlock(nn.Module):
    ...
    def __init__(self,prompt_dim=128,prompt_len=5,prompt_size = 16,lin_dim = 192,label_dim=64):
        ## for coronary
    # def __init__(self,prompt_dim=128,prompt_len=3,prompt_size = 16,lin_dim = 192,label_dim=64):
...
class Unet3D(nn.Module):
    def __init__(
        ...
        # self.promtGen = PromptGenBlock(mid_dim)
...
# single-channel-mask
    def p_losses(self, x_start, t,labels, cond=None, noise=None, **kwargs):
        ...
        mask = cond[:,-1,:,:,:].unsqueeze(1)
        mask = torch.clamp((mask+1.0)/2.0, min=0.0, max=1.0)
        count = torch.sum(mask)*c
        mask_ = 1.0 - mask
        count_ = torch.sum(mask_)*c
        noise = default(noise, lambda: torch.randn_like(x_start))
        ...
        if self.loss_type == 'l1':
            loss_total = F.l1_loss(noise, x_recon, reduction='mean')
            loss = F.l1_loss(noise*mask, x_recon*mask, reduction='sum')/count if count > 0 else torch.tensor(0.0).to(device)
            loss_ = F.l1_loss(noise*mask_, x_recon*mask_, reduction='sum')/count_ if count_ > 0 else torch.tensor(0.0).to(device)
            transformed = torch.fft.fftn(noise, dim=(2, 3, 4))
            transformed_ = torch.fft.fftn(x_recon, dim=(2, 3, 4))
            e=1e-7
            loss_logftt = F.mse_loss(torch.log(transformed.abs()**2+e), torch.log(transformed_.abs()**2+e))
            # ipdb.set_trace()
        ...
        return loss,loss_,loss_logftt, loss_total

    # ## multi-channel-mask input
    # def p_losses(self, x_start, t, labels,label_idx,cond=None, noise=None, **kwargs):
    #     """
    #     labels: 1-D long tensor, shape (B,), 取值 0–6，对应 cond 最后 7 个通道里
    #             哪个通道是“有效 mask”
    #     cond:   5-D tensor, shape (B, C_cond, D, H, W)，其中
    #             cond[:, -7:, ...] == (B, 7, D, H, W)  为所有类别 mask
    #     """
    #     b, c, d, h, w = x_start.shape            # d=f in your code
    #     device = x_start.device
    #
    #     # --------------------------------------
    #     # 1) 取出 7-channel mask，并按 labels 逐样本选中
    #     # --------------------------------------
    #     all_masks = cond[:, -7:, ...]            # (B, 7, D, H, W)
    #
    #     # labels → (B, 1, 1, 1, 1) 方便广播
    #     gather_idx = label_idx.view(-1, 1, 1, 1, 1).long()
    #     gather_idx = gather_idx.expand(-1, 1, d, h, w)   # same spatial size
    #     # ipdb.set_trace()
    #
    #     # mask: (B, 1, D, H, W)
    #     mask = torch.gather(all_masks, 1, gather_idx).float()
    #     # ipdb.set_trace()
    #     mask = torch.clamp((mask+1.0)/2.0, min=0.0, max=1.0)
    #     count = torch.sum(mask)*c
    #     mask_ = 1.0 - mask
    ...
    # def forward(self, x, *args, **kwargs):
    ## for multi-class mask -- multi-channel mask
    ...
    #     mask=torch.nn.functional.interpolate(mask, size=masked_img.shape[-3:])
    #     cc= self.conv3d(mask)
    #     # ipdb.set_trace()
    #
    #
    ...
    ## single mask
    def forward(self, img,mask,mask_3,label_idx,labels, *args, **kwargs):
        # bs = int(x.shape[0]/2)
        # img=x[:bs,...]
        # mask=x[bs:,...]
        # mask_=(1-mask).detach()
...
#                 # 获取形状并创建新的形状，将第二个维度设置为 3
#                 mask_shape = list(mask_.shape)  # 将 torch.Size 转换为列表
#                 mask_shape[1] = 7  # 修改第二个维度为 2
#                 # mask_shape[1] = 3  # 修改第二个维度为 3
#
#                 # 根据新的形状创建一个新的张量，并移动到 GPU
#                 mask = torch.zeros(torch.Size(mask_shape)).cuda()
#                 mask_3_shape = list(mask_.shape)
#                 mask_3_shape[1] = 3
#                 mask_3 = torch.zeros(torch.Size(mask_3_shape)).cuda()
#                 mask_3[:,0,:,:,:][mask_.squeeze(1)==2]= 1.0
#                 mask_3[:,0,:,:,:][mask_.squeeze(1)==3]= 1.0
#                 mask_3[:,1,:,:,:][mask_.squeeze(1)==4]= 1.0
#                 mask_3[:,1,:,:,:][mask_.squeeze(1)==5]= 1.0
#                 mask_3[:,2,:,:,:][mask_.squeeze(1)==6]= 1.0
#                 mask_3[:,2,:,:,:][mask_.squeeze(1)==7]= 1.0
#
#
#
# ##               for multi-class mask case alltumor
...
#                 sum_channel_0 = mask[:, 1, :, :, :].sum(dim=(1, 2, 3))
#                 sum_channel_1 = mask[:, 2, :, :, :].sum(dim=(1, 2, 3))
#                 sum_channel_2 = mask[:, 3, :, :, :].sum(dim=(1, 2, 3))
#                 sum_channel_3 = mask[:, 4, :, :, :].sum(dim=(1, 2, 3))
#                 sum_channel_4 = mask[:, 5, :, :, :].sum(dim=(1, 2, 3))
#                 sum_channel_5 = mask[:, 6, :, :, :].sum(dim=(1, 2, 3))
#
#                 # 初始化标签为 0
...
#                 labels = labels.cuda()
#
#                 # ##              for multi-class mask case coronary
#                 # # mask[:,1,:,:,:][mask_.squeeze(1)==3]= 1.0
#                 # # mask[:,2,:,:,:][mask_.squeeze(1)==4]= 1.0
#                 # # mask[:,0,:,:,:][mask_.squeeze(1)==0]= 1.0
#                 # # mask[:,0,:,:,:][mask_.squeeze(1)==1]= 1.0
...
#
#
#                 # if mask[:,1-label_idx,:,:,:].sum() > 0:
#                 #     print("WARNING: mask contains label{} of sum {}".format(1-label_idx, mask[:,1-label_idx,:,:,:].sum()))
#                 # else:
#                 #     print("mask only contains label{} of sum {}".format(label_idx, mask[:,label_idx,:,:,:].sum()))
#
#
#
#
...
#                     #     focus_present_mask=focus_present_mask
#                     # )
#                     loss_0,loss_1,loss_ftt,loss_total = self.model(
#                         image,
#                         mask,
#                         # mask,
#                         mask_3,
...
#                     )
#
#                     loss = loss_0 + loss_total
#
#                     # if self.step < 5000:
#                     #     loss = loss_0 + loss_1
#                     # else:
#                     #     loss = loss_0 + loss_1+loss_ftt
#
#                     self.scaler.scale(
...
#
#             log = {'loss': loss.item(),"loss_0":loss_0.item(),"loss_total":loss_total.item()}
#
