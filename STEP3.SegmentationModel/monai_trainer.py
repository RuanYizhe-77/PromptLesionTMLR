...
from TumorGeneration.utils import synthesize_early_tumor, synthesize_medium_tumor, synthesize_large_tumor, synt_model_prepare,synthesize_tumor_in_alltumor,synthesize_lesion_with_mask,synthesize_tumor_in_alltumor_mcmaskinput
import random
def train_epoch(model, loader, optimizer, scaler, epoch, loss_func, args, tumor_size_list):
    ...
    if args.is_save_synth:
        vqgan, sampler= synt_model_prepare(device = torch.device("cuda", args.rank),  organ=args.organ_model)
    # import ipdb
    # ipdb.set_trace()
    ...
    if args.is_save_synth:
        ...
        synth_dir_tiny = os.path.join(synth_dir, 'tiny')
        os.makedirs(synth_dir_tiny, exist_ok=True)
        synth_dir_tiny_img = os.path.join(synth_dir_tiny, 'img')
        os.makedirs(synth_dir_tiny_img, exist_ok=True)
        synth_dir_tiny_synth = os.path.join(synth_dir_tiny, 'synth')
        os.makedirs(synth_dir_tiny_synth, exist_ok=True)
        synth_dir_tiny_label = os.path.join(synth_dir_tiny, 'label')
        os.makedirs(synth_dir_tiny_label, exist_ok=True)
        synth_dir_small = os.path.join(synth_dir, 'small')
        os.makedirs(synth_dir_small, exist_ok=True)
        synth_dir_small_img = os.path.join(synth_dir_small, 'img')
        os.makedirs(synth_dir_small_img, exist_ok=True)
        synth_dir_small_synth = os.path.join(synth_dir_small, 'synth')
        os.makedirs(synth_dir_small_synth, exist_ok=True)
        synth_dir_small_label = os.path.join(synth_dir_small, 'label')
        os.makedirs(synth_dir_small_label, exist_ok=True)
        synth_dir_medium = os.path.join(synth_dir, 'medium')
        os.makedirs(synth_dir_medium, exist_ok=True)
        synth_dir_medium_img = os.path.join(synth_dir_medium, 'img')
        os.makedirs(synth_dir_medium_img, exist_ok=True)
        synth_dir_medium_synth = os.path.join(synth_dir_medium, 'synth')
        os.makedirs(synth_dir_medium_synth, exist_ok=True)
        synth_dir_medium_label = os.path.join(synth_dir_medium, 'label')
        os.makedirs(synth_dir_medium_label, exist_ok=True)
        synth_dir_large = os.path.join(synth_dir, 'large')
        os.makedirs(synth_dir_large, exist_ok=True)
        synth_dir_large_img = os.path.join(synth_dir_large, 'img')
        os.makedirs(synth_dir_large_img, exist_ok=True)
        synth_dir_large_synth = os.path.join(synth_dir_large, 'synth')
        os.makedirs(synth_dir_large_synth, exist_ok=True)
        synth_dir_large_label = os.path.join(synth_dir_large, 'label')
        os.makedirs(synth_dir_large_label, exist_ok=True)
    ...
    for idx, batch_data in enumerate(loader):
        ...
        print("label unique", np.unique(target.cpu().numpy()))
        ...
        tumor_samples = (target == 2).sum().item()
        if args.organ_type == 'kidney':

            tumor_samples = (target == 2).sum().item()
            print(f"Kidney samples make up {tumor_samples} of the batch.")
        total_samples = target.numel()
        tumor_percentage = (tumor_samples / total_samples) * 100
        print(f"Tumor samples make up {tumor_percentage:.2f}% of the batch.")
        ...
        for bs in range(data.shape[0]):
            ...
            if 'kidney_label' in data_name or 'liver_label' in data_name or 'pancreas_label' in data_name:
                # if random.random() > sample_thresh and args.is_save_synth:
                if args.is_save_synth:
                    healthy_data = data[bs][None,...]
                    healthy_target = target[bs][None,...]
                    # tumor_types = ['early', 'medium', 'large']

                    ## for tumor synthesis with mcmask-transformer
                    synt_data, synt_target,synthetic_tumor_type = synthesize_tumor_in_alltumor(healthy_data, healthy_target, args.organ_type, vqgan, sampler)
                    # ## for tumor synthesis with mcmask input
                    # synt_data, synt_target,synthetic_tumor_type = synthesize_tumor_in_alltumor_mcmaskinput(healthy_data, healthy_target, args.organ_type, vqgan, sampler)
                    ## for coronary artery
                    # synt_data, synt_target,recon_volume,sample_,final_volume_with_recon,masked_sample,masked_recon,masked_real,synthetic_tumor_type \
                    #     = synthesize_lesion_with_mask(healthy_data, healthy_target, args.organ_type, vqgan, sampler)
                    # ipdb.set_trace()
                    synth_dir_f_img = synth_dir
                    synth_dir_f_label = synth_dir
                    if synthetic_tumor_type == 'tiny':
                        synth_dir_f_img = synth_dir_tiny_img
                        synth_dir_f_synth = synth_dir_tiny_synth
                        synth_dir_f_label = synth_dir_tiny_label
                    elif synthetic_tumor_type == 'small':
                        synth_dir_f_img = synth_dir_small_img
                        synth_dir_f_synth = synth_dir_small_synth
                        synth_dir_f_label = synth_dir_small_label
                    elif synthetic_tumor_type == 'medium':
                        synth_dir_f_img = synth_dir_medium_img
                        synth_dir_f_synth = synth_dir_medium_synth
                        synth_dir_f_label = synth_dir_medium_label
                    elif synthetic_tumor_type == 'large':
                        synth_dir_f_img = synth_dir_large_img
                        synth_dir_f_synth = synth_dir_large_synth
                        synth_dir_f_label = synth_dir_large_label
                # if args.is_save_synth:
                    ...
                    #     synt_data, synt_target = synthesize_early_tumor_in_alltumor(healthy_data, healthy_target, args.organ_type, vqgan, sampler)
                    # elif synthetic_tumor_type == 'medium':
                    #     synt_data, synt_target = synthesize_early_tumor_in_alltumor(healthy_data, healthy_target, args.organ_type, vqgan, sampler)
                    # elif synthetic_tumor_type == 'large':
                    #     synt_data, synt_target = synthesize_early_tumor_in_alltumor(healthy_data, healthy_target, args.organ_type, vqgan, sampler)
        ...
        with autocast(enabled=args.amp):
            ...
            # tumor_loss = loss_func(torch.stack([tumor_logits[:,0],tumor_logits[:,3]], dim=1), target==2)
    ...
    # import ipdb
        ...
        if args.rank==0:
            print('Epoch {}/{} {}/{}'.format(epoch, args.max_epochs, idx, len(loader)),
                  ...
                  # 'plaqueloss: {:.4f}'.format(palqueloss.item()), #for plaque
                  # 'tumor_loss: {:.4f}'.format(tumor_loss.item()), # for tumor
