...
from monai.transforms import AsDiscrete,Activations,Compose,RandCropByLabelClassesd
...
from TumorGeneration.utils import synthesize_early_tumor, synthesize_medium_tumor, synthesize_large_tumor, synt_model_prepare,synthesize_lesion_with_mask,synthesize_alltumor_with_mask
import random
import time
...
parser.add_argument('--synth_dir', default='/data/utsubo0/users/ruan/synth_alltumor_mcmask/', type=str)
...
# parser.add_argument('--distributed',  default=False, type=bool) #distributed multi gpu
...
parser.add_argument('--label_idx', default=2, type=int)
    ...
    train_transform = transforms.Compose(
        [
            ...
            RandCropByLabelClassesd(
                ...
                ratios=[0, 0.5,0.5,0,0],
                # ratios=[0, 0.1,0.1,1.0, 0.0] if args.label_idx == 0 else [0, 0.1,0.1,0,1],
                num_classes=5,
                num_samples=1,
                image_key="image",
                image_threshold=-1,
            ...
            #     label_key="label",
            #     spatial_size=(args.roi_x, args.roi_y, args.roi_z), #192, 192, 64
            #     ratios=[0.01, 0.01, 5.0, 5.0, 5.0, 5.0, 5.0, 5.0] ,
            #     num_classes=8,
            #     num_samples=5,
            #     image_key="image",
            #     image_threshold=-1,
...
def main_worker(gpu, args):
    ...
    if 'coronary' in datafold_dir:
        for line in open(os.path.join(datafold_dir, 'syth_lesion_train_{}.txt'.format(args.fold))):
            name = line.strip().split('\t')[0]

            train_img_healthy.append(args.data_root + name+ '/ct.nii.gz')
            train_lbl_healthy.append(args.data_root + name+ '/segmentations/plaque.nii.gz')
            train_name_healthy.append(name)
    else:
        for line in open(os.path.join(datafold_dir, 'syth_tumor_train_{}.txt'.format(args.fold))):
            name = line.strip().split()[1].split('.')[0]
            train_img_healthy.append(line.strip().split()[0])
            train_lbl_healthy.append(line.strip().split()[1])
            train_name_healthy.append(name)
    ...
    #     name = line.strip().split('\t')[0]
    #
    #     train_img_healthy.append(data_root + name+ '/ct.nii.gz')
    #     train_lbl_healthy.append(data_root + name+ '/segmentations/plaque.nii.gz')
    #     train_name_healthy.append(name)
    ...
    train_ds = monai_data.CacheDataset(data=data_dicts_train, transform=train_transform, cache_rate=args.cache_rate)
    ...
    if args.organ_type == 'liver':
        ...
        sample_thresh = 0.5
    # model prepare
    vqgan, early_sampler= synt_model_prepare(device = torch.device("cuda", args.rank), label_idx=args.label_idx, organ=args.organ_model)

    os.makedirs("synth_data", exist_ok=True)
    def reverse_scale_intensity(processed_image, a_min, a_max, b_min, b_max):
        # 使用反向公式将处理后的值恢复到原始范围
        original_value = (processed_image - b_min) / (b_max - b_min) * (a_max - a_min) + a_min
        return original_value
    import nibabel as nib

    if args.is_save_synth:
        os.makedirs(args.synth_dir, exist_ok=True)
        synth_dir = os.path.join(args.synth_dir, args.organ_type)
        os.makedirs(synth_dir, exist_ok=True)

        synth_dir_img = os.path.join(synth_dir, 'img')
        os.makedirs(synth_dir_img, exist_ok=True)
        synth_dir_synth = os.path.join(synth_dir, 'synth')
        os.makedirs(synth_dir_synth, exist_ok=True)
        synth_dir_lbl = os.path.join(synth_dir, 'lbl')
        os.makedirs(synth_dir_lbl, exist_ok=True)
        synth_dir_lbl_real = os.path.join(synth_dir, 'lbl_real')
        os.makedirs(synth_dir_lbl_real, exist_ok=True)
    ...
    for idx, batch_data in enumerate(train_loader):
        ...
        # synt_data, synt_target,recon_data,synt_data_ori,synt_data_with_recon,masked_sample,masked_recon,masked_real = synthesize_alltumor_with_mask(data, target, vqgan, early_sampler,idx,args)

        synt_data, synt_target,mask_organ,recon_data,synt_data_ori,synt_data_with_recon,masked_sample,masked_recon,masked_real = synthesize_lesion_with_mask(data, target, vqgan, early_sampler,idx,args)
        # data_f = reverse_scale_intensity(data,-175.0,250.0,-1.0,1.0)
        # synt_data_f = reverse_scale_intensity(synt_data,-175.0,250.0,-1.0,1.0)
        # ipdb.set_trace()




        ##save coronary synth for validation
        for i in range(len(data)):
            for bs in range(len(data[i])):
                nii_img = nib.Nifti1Image(data[i][bs].detach().cpu().numpy(), affine=np.eye(4))
                nii_img.to_filename(os.path.join(synth_dir_img,f"{idx}_{i}_{bs}.nii.gz"))
                nii_img = nib.Nifti1Image(synt_data[i][bs].detach().cpu().numpy(), affine=np.eye(4))
                nii_lbl_real = nib.Nifti1Image(mask_organ[i][bs], affine=np.eye(4))
                nii_lbl_real.to_filename(os.path.join(synth_dir_lbl_real,f"{idx}_{i}_{bs}.nii.gz"))
                nii_lbl = nib.Nifti1Image(synt_target[i][bs].detach().cpu().numpy(), affine=np.eye(4))
                nii_img.to_filename(os.path.join(synth_dir_synth,f"{idx}_{i}_{bs}.nii.gz"))
                nii_lbl.to_filename(os.path.join(synth_dir_lbl,f"{idx}_{i}_{bs}.nii.gz"))



            # nii_img = nib.Nifti1Image(healthy_data[0][0].detach().cpu().numpy(), affine=np.eye(4))
            # nii_synth = nib.Nifti1Image(synt_data[0][0].detach().cpu().numpy(), affine=np.eye(4))
            # nii_lbl = nib.Nifti1Image(synt_target[0][0].detach().cpu().numpy(), affine=np.eye(4))
            # nii_img.to_filename(os.path.join(synth_dir_f_img,f"{epoch}_{idx}_{bs}.nii.gz"))
            # nii_synth.to_filename(os.path.join(synth_dir_f_synth,f"{epoch}_{idx}_{bs}.nii.gz"))
            # nii_lbl.to_filename(os.path.join(synth_dir_f_label,f"{epoch}_{idx}_{bs}.nii.gz"))

        # ##test coronary synth
        # for i in range(len(data)):
        #
        #     nii_img = nib.Nifti1Image(data[i][0].detach().cpu().numpy(), affine=np.eye(4))
        #     nii_lbl = nib.Nifti1Image(target[i][0].detach().cpu().numpy(), affine=np.eye(4))
        #     nii_img.to_filename(f"synth_data/{idx}_{i}_img.nii.gz")
        #     nii_lbl.to_filename(f"synth_data/{idx}_{i}_lbl.nii.gz")
        #     nii_img = nib.Nifti1Image(synt_data[i][0].detach().cpu().numpy(), affine=np.eye(4))
        #     nii_lbl = nib.Nifti1Image(synt_target[i][0].detach().cpu().numpy(), affine=np.eye(4))
        #     nii_img.to_filename(f"synth_data/{idx}_{i}_img_synth_lesion_{args.label_idx}.nii.gz")
        #     nii_lbl.to_filename(f"synth_data/{idx}_{i}_lbl_synth.nii.gz")
        #     nii_img = nib.Nifti1Image(recon_data[i][0].detach().cpu().numpy(), affine=np.eye(4))
        #     nii_img.to_filename(f"synth_data/{idx}_{i}_img_recon.nii.gz")
        #     nii_img = nib.Nifti1Image(synt_data_ori[i][0].detach().cpu().numpy(), affine=np.eye(4))
        #     nii_img.to_filename(f"synth_data/{idx}_{i}_img_orisynth_lesion_{args.label_idx}.nii.gz")
        #     nii_img = nib.Nifti1Image(synt_data_with_recon[i][0].detach().cpu().numpy(), affine=np.eye(4))
        #     nii_img.to_filename(f"synth_data/{idx}_{i}_img_synth_with_recon_lesion_{args.label_idx}.nii.gz")
        #     for j in range(len(masked_sample)):
        #         nii_img = nib.Nifti1Image(masked_sample[j][i][0].detach().cpu().numpy(), affine=np.eye(4))
        #         nii_img.to_filename(f"synth_data/{idx}_{i}_masked_sample_{j}.nii.gz")
        #         nii_img = nib.Nifti1Image(masked_recon[j][i][0].detach().cpu().numpy(), affine=np.eye(4))
        #         nii_img.to_filename(f"synth_data/{idx}_{i}_masked_recon_{j}.nii.gz")
        #         nii_img = nib.Nifti1Image(masked_real[j][i][0].detach().cpu().numpy(), affine=np.eye(4))
        #         nii_img.to_filename(f"synth_data/{idx}_{i}_masked_real_{j}.nii.gz")
