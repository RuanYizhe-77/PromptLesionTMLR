...
class VQGAN(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        #     recon_loss, x_recon, vq_output, perceptual_loss = self.forward(volume)
        ...
        #     import numpy as np
        #     nii_img = nib.Nifti1Image(recon_volume[0][0].detach().cpu().numpy().astype(np.float32), affine=np.eye(4))
        #     nii_img.to_filename(f"synth_data/{batch_idx}_img_recon.nii.gz")
        #     nii_img = nib.Nifti1Image(volume[0][0].detach().cpu().numpy().astype(np.float32), affine=np.eye(4))
        #     nii_img.to_filename(f"synth_data/{batch_idx}_img.nii.gz")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            nii_img = nib.Nifti1Image(volume[0][0].detach().cpu().numpy().astype(np.float32), affine=np.eye(4))
            nii_img.to_filename(f"synth_data/{batch_idx}_img.nii.gz")
        # recon_loss,recon_x, vq_output, perceptual_loss = self.forward(x)
